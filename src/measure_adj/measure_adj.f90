...
subroutine measure_adj(data_in,npt1_in,t01_in,dt1_in, &
  ...
  double precision, intent(in) :: data_in(*), syn_in(*), syn_phydisp_in(*)
  double precision :: t01_in, dt1_in, t02_in, dt2_in, t03_in, dt3_in
  ...
  integer,intent(in) :: weighting_option

  double precision :: adj_source(:)
  ...
  !copy from argument list to local var(because the data type is different)
   ...
   enddo ! nmeas
    ...
    ! write out the CC-reconstructed data from synthetics
      ...
      adj_source(1:nn)=adj_syn_all(1:nn)
...
subroutine copy_argu_to_local_and_check(data_in,npt1_in,t01_in,dt1_in, &
  ...
  if(abs(dt1-dt2) > TOL) then
    print *, 'ERROR: check if dt match(data and syn)'
    nerr = 1
    return
  endif
  dt=dt1
  ...
  if(abs(t01-t02) > dt) then
    print *, 'Check if t0 match(data and syn)'
    nerr = 1
    return
  endif
  t0=t01

  data(1:npt1)=data_in(1:npt1)
  syn(1:npt2)=syn_in(1:npt2)
...
!-----------------------------------------------------'
subroutine read_ma_parfile(measure_adj_par_all,min_p,max_p,event_dpt)

  use ma_struct

  type(ma_par_struct_all) :: measure_adj_par_all
  double precision,intent(in) :: min_p, max_p, event_dpt

  integer :: min_p_int, max_p_int
  character(len=10) :: min_p_string, max_p_string
  character(len=150) :: fn,parfile_dir
  ...
  write(max_p_string,'(I6)') int(max_p)
  write(min_p_string,'(I6)') int(min_p)
  max_p_string=adjustl(max_p_string)
  min_p_string=adjustl(min_p_string)
  !print *,"fstart_string, fend_string:", fstart_temp, fend_temp

  parfile_dir="PAR_FILES"

  fn=trim(parfile_dir)//"/MA_PARFILE_"//trim(min_p_string)//"_"//&
        trim(max_p_string)//"_Z"
  print *, "Reading in ma parfile in Z component"
  print *, "Parfile name:", trim(fn)
  call read_ma_parfile_comp(fn, measure_adj_par_all%Z) 
  
  fn=trim(parfile_dir)//"/MA_PARFILE_"//trim(min_p_string)//"_"//&
        trim(max_p_string)//"_R"
  print *, "Reading in ma parfile in R component"
  print *, "Parfile name:", trim(fn)
  call read_ma_parfile_comp(fn, measure_adj_par_all%R) 
  
  fn=trim(parfile_dir)//"/MA_PARFILE_"//trim(min_p_string)//"_"//&
        trim(max_p_string)//"_T"
  print *, "Reading in ma parfile in T component"
  print *, "Parfile name:", trim(fn)
...
!-------------------------------------------------'
subroutine copy_ma_par_to_local(ma_par_all,chan_dat,fstart0,fend0,tt,dtt,nn,chan)
  ...
  select case (chan_dat(3:3))
    ...
    case ("T")
      ma_par=ma_par_all%T
    case default
      print *, "Check you component, no par match!"
      print *, "Component: ", trim(chan_dat(3:3))
      stop
  end select
  ...
  !fstart0 = 1.0/TLONG; fend0 = 1.0/TSHORT
  ...
  !if( fstart0 >= fend0 ) &
  ! stop 'Error: change input frequency range of the signal'

  !if( nn > NDIM ) &
  !  stop 'Error: Change interpolation nn or NDIM'

  !if( imeas == 1 .or. imeas==2)
