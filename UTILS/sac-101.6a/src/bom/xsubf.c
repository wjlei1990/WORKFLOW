...
xsubf(int *nerr) {
	...
	*nerr = 0;
	...
	/* - Check to make sure all files are 
	 *   evenly spaced time series files. */
	vfeven( nerr );
	if( *nerr != 0 )
	    goto L_8888;
	...
	/* EXECUTION PHASE: */
	/* - Perform the file subtraction on each file in DFL. */
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
	    ...
	    *delta = delta2;
	    ...
	    /* -- Perform file subtraction on these two files. */
