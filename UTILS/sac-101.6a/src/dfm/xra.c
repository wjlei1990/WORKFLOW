...
/** 
 * Execute the command READALPHA (RA) to read a alpha sac file
 * 
 * @param lplot 
 *    - TRUE plot the file
 *    - FALSE do not plot the file
 * @param nerr 
 *    Error Return Flag
 *    - 0 on Success
 *    - ERROR_NO_DATA_FILES_READ_IN
 *    - ERROR_MULTIPLE_X_CHANNELS_IN_FILE
 *    - ERROR_MISSING_Y_CHANNEL_IN_FILE
 *
...
xra(int  lplot, 
	...
	int ipt, itype, jdx, jch, jcomp, jdfl, jen, nc, nc1, nc2, ncerr;
	int nchar, ndcont, ndflin, ndflrq, ndflsv, ndform, ndxch[MAXCH-(0)+1];
	int nentry, newlen, newndx, nlnch[MAXCH-(0)+1], notused;
	int nptch[MAXCH-(0)+1], nsndfl, numch, numxch, numych ;
        FILE *nun;
...
L_3700:
		...
		*nerr = ERROR_MULTIPLE_X_CHANNELS_IN_FILE;
		setmsg( "ERROR", *nerr );
		...
		*nerr = ERROR_MISSING_Y_CHANNEL_IN_FILE;
		setmsg( "ERROR", *nerr );
		goto L_8888;
	    }
...
L_5000:
	...
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
        ...
        itype = 0;
	    if( itype == 0 ){
		fprintf( MUNOUT, "xra: no more tokens in file name list\n" );
	    }
	}
